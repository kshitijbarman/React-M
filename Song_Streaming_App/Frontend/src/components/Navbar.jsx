...
const Navbar = () => {
  ...
  return (
    ...
    <div className={`fixed top-0 left-0 right-0 z-50 flex justify-between items-center p-3 px-5 shadow-md ${toggle ? 'bg-[#f5f5f5]' : 'bg-black text-white'}`}>
        ...
        <div className="lg:flex items-center gap-4 ">
            <div className="flex items-center ">
                   <div className=' pr-5 text-2xl'><MdDarkMode onClick={prevalue}/></div>
                <div className="flex flex-col text-sm font-semibold  ">
                    <span className={`text-gray-600 text-[14px] md:text-lg flex flex-col text-sm font-semibold ${toggle?'bg-[#f5f5f5]':'bg-black text-white'}`}>Music Languages</span>
                    <span className={`text-gray-500 text-[12px] hidden ${toggle?'bg-[#f5f5f5]':'bg-black text-white'} lg:block`}>Hindi</span>
                </div>
                <MdKeyboardArrowDown className="text-xl text-gray-500 ml-1" />
            </div>

            <ul className={`hidden lg:flex gap-5 text-gray-600 font-semibold ${toggle?'bg-[#f5f5f5]':'bg-black text-white'}`}>
